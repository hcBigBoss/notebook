...
- term查询：根据词条精确匹配【不分词】，一般搜索keyword类型、数值类型、布尔类型、日期类型字段
...
```json
#geo_distance 查询，以点为圆心，向周边辐射多少距离
GET /hotel/_search
{
  "query": {
    "geo_distance": {
...
> - 搜索的结果与搜索的内容相匹配，匹配度越高，分值就越高，排位就越高
...
- 过滤条件【filter】：哪些文档要加分
- 算分函数【weight】：如何计算function score
- 加权方式【sum】：function score 与 query score如何运算
...
```
#搜索名字包含“如家”，价格不高于400，在坐标
#31.21,121.5周围10km范围内的酒店
GET /hotel/_search
{
  "query": {
    "bool": {
...
- `scroll`：
  ...
  - 场景：海量数据的获取和迁移。从ES7.1开始不推荐，建议用 search after 方案。
...
> SearchRequest.source().query( 查询条件【QueryBuilders.xxxQuery()】 )
...
> SearchResponse.getHits().getHits()    得到集合结果
 ...
 */
@Test
public void testBoolAndPageAndSort() throws Exception {
 ...
 */
@Test
public void testBoolAndPageAndSortAndHighLight() throws Exception {
    ...
    for (SearchHit hit : hits) {
        ...
        if(highlightFields != null && highlightFields.size() > 0) {
            ...
            if(highlightField != null) {
                //获取高亮字段name的值，并转成字符串
                String name = highlightField.getFragments()[0].toString();  //["<em>如家</em>酒店(北京良乡西路店)"]
                hotelDoc.setName(name);   //把高亮的值放到文档中
            }
        }
        System.out.println("hotelDoc = " + hotelDoc);
    }
...
![1659922095689](assets/1659922095689.png)
...
<img src="assets/image-20210722101908062.png" alt="image-20210722101908062" style="zoom:80%;" />
...
![1659923545097](assets/1659923545097.png)
