...
public Map<String, List<String>> filters(RequestParams params) {
    ...
    try {
        ...
        //过滤条件
        queryCondition(params, request);

        //2.聚合
        //2.1 品牌聚合
        aggByName(request, "brandAgg", "brand", 10);
        //2.2 星级聚合
        aggByName(request, "starNameAgg", "starName", 10);
        //2.3 城市聚合
        aggByName(request, "cityAgg", "city", 10);
        ...
        List<String> cityList = getListByAggName(response, "cityAgg");
        resultMap.put("city", cityList);

        List<String> starNameList = getListByAggName(response, "starNameAgg");
...
      "name": {
        "type": "text",
        "analyzer": "my_analyzer",   #分词时使用的分词器
...
- ②解压并放到elasticsearch的 es-plugin插件对应的目录
...
- 为了避免搜索到同音字，搜索时不要使用拼音分词器【使用ik分词器】
...
public List<String> suggestion(String key) {
    try {
        ...
        request.source().suggest(
                new SuggestBuilder()
                        //String name, SuggestionBuilder<?> suggestion
                        .addSuggestion(
                                "hotel_suggest",
        ...
        CompletionSuggestion suggestion = response.getSuggest().getSuggestion("hotel_suggest");
